...
func _clear_slot(slot: Control, index: int, emoji_label: Label) -> void:
	_slot_data[index] = {}
	slot.get_node("Glow").visible = false
	slot.get_node("Icon").visible = false
	emoji_label.text = ""
...
func _check_recipe() -> void:
	...
	var result_id := GameData.lookup_recipe(
		_slot_data[0].get("id", ""),
		_slot_data[1].get("id", "")
	)
...
func _shake_result_slot() -> void:
	...
	_shake_tween.tween_property(_result_slot, "rotation", strength, 0.04).set_trans(Tween.TRANS_SINE)
	_shake_tween.tween_property(_result_slot, "rotation", -strength, 0.08).set_trans(Tween.TRANS_SINE)
	_shake_tween.tween_property(_result_slot, "rotation", strength * 0.55, 0.07).set_trans(Tween.TRANS_SINE)
	_shake_tween.tween_property(_result_slot, "rotation", 0.0, 0.06).set_trans(Tween.TRANS_SINE)
